...
def print_posting(file, posting_line, tid, block_dict):
  if not posting_line:
    return
  posting_line_length = len(posting_line)
  block_dict[tid] = (file.tell(), posting_line_length)
  file.write(struct.pack('i'*posting_line_length, *posting_line))
...
while True:
  ...
  while ptr1 < len(b1_tids) and ptr2 < len(b2_tids):
    ...
    freq1 = dict1[tid1][1]
    freq2 = dict2[tid2][1]
    b1_f.seek(dict1[tid1][0])
    b2_f.seek(dict2[tid2][0])
    line1 = struct.unpack('i' * freq1, b1_f.read(4 * freq1))
    line2 = struct.unpack('i' * freq2, b2_f.read(4 * freq2))
    if tid1 < tid2:
