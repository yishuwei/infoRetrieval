...
def read_posting(term_id):
  start_pos = file_pos_dict[term_id]
  index_f.seek(start_pos)
  if (term_id + 1) not in file_pos_dict:
    return gamma.getPostings(index_f.read())
  
  return gamma.getPostings(index_f.read(file_pos_dict[term_id + 1] - start_pos))
