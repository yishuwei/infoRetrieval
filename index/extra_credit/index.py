...
def print_posting(file, posting_line, tid, block_dict):
  ...
  file.write(gamma.convertPostings(posting_line))
...
while True:
  ...
  while ptr1 < len(b1_tids) and ptr2 < len(b2_tids):
    ...
    if (ptr1 + 1) == len(b1_tids):
      line1 = gamma.getPostings(b1_f.read())
    else:
      line1 = gamma.getPostings(b1_f.read(dict1[b1_tids[ptr1 + 1]][0] - start_pos1))
    
    if (ptr2 + 1) == len(b2_tids):
      line2 = gamma.getPostings(b2_f.read())
    else:
      line2 = gamma.getPostings(b2_f.read(dict2[b2_tids[ptr2 + 1]][0] - start_pos2))
